 ...
 */
export default async function some<T> (arr: T[], callback: (value: T, index: number, array: T[]) => Promise<boolean>): Promise<boolean> {
  for (let currentIndex = 0, length = arr.length; currentIndex < length; currentIndex++) {
    if (await callback(arr[currentIndex], currentIndex, arr)) return true
  }
